...
const Sidebar = ({filters, onFilterUpdate, onFilterSubmit}) => {
  ...
  const setFilter = (key, value) => {
    onFilterUpdate({...filters, [key]: value});
  };

  const setFilterArray = (key, checked, value) => {
    if(checked) {
      onFilterUpdate({...filters, [key]: [...filters[key], value]});
    } else {
      ...
      onFilterUpdate({...filters, [key]: filterArray});
    }
    console.log(filters)
  };
              ...
              {categories && categories.map((category) => (
                <div className="flex items-center gap-2" key={category}>
                  <input
                    ...
                    onChange={(event) => setFilterArray("categories", event.target.checked, category)}
                  />
                  <label
            ...
            {productConditions && productConditions.map((productCondition) => (
                <div className="flex items-center gap-2" key={productCondition}>
                  <input
                    ...
                    onChange={(event) => setFilterArray("conditions", event.target.checked, productCondition)}
                  />
