    ...
    <Link to={`/products/${product.productId}`} className="bg-white rounded-lg shadow-md overflow-hidden h-96">
      ...
      <div className="flex flex-col justify-between p-4 max-h-48 h-full">
        ...
        <div className="text-gray-800 text-xs font-medium mb-2">
          <span className="bg-gray-100 h-6 px-2.5 py-0.5 rounded-full border border-gray-400 mr-1">{product.categoryName}</span>
          <span className="bg-gray-100 h-6 px-2.5 py-0.5 rounded-full border border-gray-400 mr-1">{product.productCondition}</span>
          {product.shippingType === 'free' ? (<span className="bg-gray-100 h-6 px-2.5 py-0.5 rounded-full border border-gray-400">Free shipping</span>) : <></> }
