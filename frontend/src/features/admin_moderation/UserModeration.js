...
export default function UserModeration() {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await axios.get("http://localhost:8080/admin/users/all");
  ...
  const handleSaveUser = async (updatedUser) => {
    try {
      await axios.put(`http://localhost:8080/admin/users/${updatedUser.userId}`, updatedUser);
  ...
  const handleActivateDeactivate = async (userId) => {
    try {
      ...
      setUsers((prevUsers) => prevUsers.map((user) => (user.userId === userId ? updatedUser : user)));
